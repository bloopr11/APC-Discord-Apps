...
from db_logger import (
    ...
    get_training_data, close_db_pool
)
...
from tv_connection_pool import init_tv_pool, close_tv_pool, fetch_tv_ws_sync
    ...
    def _bg_retrain():
        with _retrain_lock:
            try:
                ...
                if rows:
                    maybe_retrain(rows)
                    print(f"[ML] Background retrain completed with {len(rows)} rows")
...
# ==================================================
def cleanup_figure(fig, close_all=False):
    try:
        if fig is not None:
            plt.close(fig)
            if hasattr(fig, 'axes'):
...
# ==================================================
# DATA FETCHING WITH TV POOL OR YF
# ==================================================
...
def candle_confirmation(df: pd.DataFrame, div_type: str) -> dict:
    ...
    if div_type == "BULLISH_DIV":
        ...
        desc = (f"Candle: {'Hijau' if bull_candle else 'Merah'}  "
        ...
        desc = (f"Candle: {'Merah' if bear_candle else 'Hijau'}  "
                f"Body: {body_pct*100:.0f}%  "
...
def advanced_score(
    ...
    if lstm == "BULLISH":
        ...
        t -= 10
    if ema_conf["BULLISH_STACK"]:
        t += 8
    elif ema_conf["BEARISH_STACK"]:
        t -= 8
    if ema_conf["NEAR_200EMA"]:
        t += 3
    if trend["trending"]:
    ...
    if of["bias"] == "BUY":
        ...
        o -= 8
    if of["buy_vol_pct"] > 60:
        o += 4
    elif of["buy_vol_pct"] < 40:
        o -= 4
    if of["absorption"]:
        o += 3
    if vol_ratio >= 1.5:
    ...
    if fvg["in_bull_fvg"]:
        f += 10
    elif fvg["in_bear_fvg"]:
        f -= 10
    elif fvg["count_bull"] > 0:
        f += 3
    elif fvg["count_bear"] > 0:
        f -= 3
...
def confidence_label(score: int, confluence: int = 0) -> str:
    if score >= 82 and confluence >= 5:
        return "🔥 VERY HIGH"
    if score >= 70 and confluence >= 4:
        return "✅ HIGH"
    if score >= 55:
        return "⚠️ MODERATE"
    return "❌ LOW"
...
# ==================================================
# GENERATE SIGNAL — with safe retrain and ML
# ==================================================
def generate_signal(pair: str, timeframe: str = DEFAULT_TF) -> dict:
    ...
    bb_pos = float((close.iloc[-1] - bb_lower.iloc[-1]) / bb_range if bb_range else 0.5)
    ...
    # ML Adaptive Scoring
    _signal_draft = {
        "pair": pair, "timeframe": timeframe, "score": rule_score,
        "rule_score": rule_score, "cats": sc["cats"], "rsi": rsi_val,
        "stoch_k": stoch_k_val, "bb_pos": bb_pos, "vol_ratio": vol_ratio_val,
        "trend": trend, "action": action, "ema": ema_conf,
        "orderflow": of, "mtf": mtf,
    }
    ...
    result = {
        "pair": pair, "timeframe": timeframe, "score": score,
        "rule_score": rule_score, "cats": sc["cats"], "confluence": sc["confluence"],
        "confidence": confidence_label(score, sc["confluence"]),
        "action": action, "entry": price, "tp": sl_data["tp2"], "sl": sl_data["sl2"],
        "tp1": sl_data["tp1"], "tp2": sl_data["tp2"], "sl1": sl_data["sl1"],
        "sl2": sl_data["sl2"], "rr1": sl_data["rr1"], "rr2": sl_data["rr2"],
        "rr": sl_data["rr2"], "rsi": rsi_val, "macd": macd_hist_val,
        "atr": atr_val, "stoch_k": stoch_k_val, "bb_pos": bb_pos,
        "vol_ratio": vol_ratio_val, "rsi_div": rsi_div, "ema": ema_conf,
        "lstm": lstm, "smc": smc_data, "liq": liq, "flow": fl,
        "trend": trend, "orderflow": of, "fvg": fvg, "session": sess,
        "mtf": mtf, "sweep": sl_data, "regime": regime_data, "ml": ml_result,
        "df": df,
    }

    # Persist to DB + background retrain (thread-safe)
    try:
        log_signal(result)
...
def build_embed(data: dict) -> discord.Embed:
    ...
    ml_tag = f" _(rule: {rule_s:.0f})_" if abs(data["score"] - rule_s) >= 1 else ""
    embed = discord.Embed(
        title=f"{action_emoji} {data['pair']} — {data['action']} SIGNAL  [{data['confluence']}/6 confluence]",
        description=f"**AI Score:** `{data['score']:.1f}%`{ml_tag} — {data['confidence']}\n"
    ...
    embed.add_field(name="🕐 MTF Bias", value="\n".join(htf_lines) if htf_lines else "N/A", inline=True)
    ...
    if "regime" in data:
        ...
        embed.add_field(name=f"{data['regime']['emoji']} Market Regime", value=regime_val, inline=False)
    if "ml" in data:
        embed.add_field(name="🤖 ML Adaptive Score", value=ml_embed_value(data["ml"]), inline=False)
    embed.set_footer(text=f"⏱ TF: {data['timeframe'].upper()} | MTF: {'+'.join(mtf['htf_list'])} | AI Institutional Engine v4")
    return embed

# ==================================================
# CREATE CHART (with memory cleanup)
# ==================================================
def create_chart(
    df: pd.DataFrame,
    pair: str,
    ...
    gs = fig.add_gridspec(2, 1, height_ratios=[5, 1], hspace=0.06, left=0.01, right=0.86, top=0.93, bottom=0.08)
    ...
    if is_buy:
        ...
        if tp1:
            ax.axhspan(entry, tp1, alpha=0.14, color="#69f0ae", zorder=1)
    else:
        ax.axhspan(tp, entry, alpha=0.08, color="#00c853", zorder=1)
        if tp1:
            ax.axhspan(tp1, entry, alpha=0.14, color="#69f0ae", zorder=1)
    if is_buy:
        ax.axhspan(sl, entry, alpha=0.10, color="#d50000", zorder=1)
        if sl1:
            ax.axhspan(sl1, entry, alpha=0.14, color="#ff6d00", zorder=1)
    else:
        ax.axhspan(entry, sl, alpha=0.10, color="#d50000", zorder=1)
        if sl1:
            ax.axhspan(entry, sl1, alpha=0.14, color="#ff6d00", zorder=1)
    if sweep_data:
    ...
    # ... (you can keep the rest of your existing chart code, including annotations and legend)
    # The key is the cleanup at the end.
    filename = f"{pair}_candles.png"
    plt.savefig(filename, dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
    cleanup_figure(fig, close_all=False)
    return filename

# ==================================================
# CREATE DIV CHART (with memory cleanup)
# ==================================================
...
# ==================================================
# TV SEMAPHORE (for concurrency control)
# ==================================================
def _get_tv_semaphore():
    return asyncio.Semaphore(3)  # Max 3 concurrent TV requests
...
def _rsi_div_detail(df: pd.DataFrame, rsi_series: pd.Series, lookback: int = 5) -> dict:
    ...
    if price_diff < 0 and rsi_diff > 0:
        ...
        strength = 0.0
    return {
        "type": div_type, "strength": strength,
        "price_now": price_now, "price_prev": price_prev,
        "rsi_now": rsi_now, "rsi_prev": rsi_prev,
        "price_diff": price_diff, "rsi_diff": rsi_diff,
        "lookback": lookback,
    }
...
def create_div_chart(df, pair, timeframe, div, signal_data) -> str:
    # Reuse create_chart structure or implement specifically
    # This is a simplified version; replace with your full implementation
    df_c = df.copy()
    if not isinstance(df_c.index, pd.DatetimeIndex):
        df_c.index = pd.to_datetime(df_c.index)
    candle_limit = {"5m": 80, "15m": 80, "1h": 72, "4h": 48}
    ...
    sl2 = signal_data["sl2"]
    fig = plt.figure(figsize=(18, 11), facecolor="#0d1117")
    gs = GridSpec(2, 1, figure=fig, height_ratios=[3, 1.2], hspace=0.04, left=0.01, right=0.85, top=0.93, bottom=0.07)
    ax_c = fig.add_subplot(gs[0])
    ax_r = fig.add_subplot(gs[1])
    ax_c.set_facecolor("#0d1117")
    ax_r.set_facecolor("#0d1117")
    # Plot price action and levels similarly to create_chart
    # ... (abbreviated for brevity, but should copy your full logic)
    filename = f"{pair}_div_chart.png"
    plt.savefig(filename, dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
    cleanup_figure(fig, close_all=False)
    return filename

async def check_and_send_div(channel, pair: str, timeframe: str):
    global _div_sent
...
# ==================================================
# AUTO SIGNAL LOOP (with memory cleanup)
# ==================================================
                    ...
                    try:
                        ...
                        if data["score"] >= AUTO_MIN_SCORE:
                            ...
                            try:
                                ...
                                pass
                            chart_counter += 1
                            if chart_counter % 10 == 0:
                                cleanup_figure(None, close_all=True)
                                print(f"[MEMORY] Cleaned after {chart_counter} charts")
                    except Exception as e:
