...
class PairSelectView(View):
    ...
    async def on_pair_select(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(
            f"⏳ Memproses `{pair}` `{self.timeframe.upper()}`...", ephemeral=True
        )
...
class SignalActionView(View):
    ...
    @discord.ui.button(label="📋 Ganti TF", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(label="🏆 Best Signal", style=discord.ButtonStyle.success)
...
# ==================================================
# OUTCOME VIEWS  (step-by-step tanpa ketik manual)
# ==================================================

class OutcomePairView(View):
    """Step 1: pilih pair."""
    def __init__(self):
        super().__init__(timeout=60)
        options = [
    ...
    async def on_pair(self, interaction: discord.Interaction):
        pair = interaction.data["values"][0]
        view = OutcomeTFView(pair)
        await interaction.response.edit_message(
            content=f"📋 **{pair}** | Pilih timeframe:", view=view
        )
...
class OutcomeTFView(View):
    ...
    async def on_tf(self, interaction: discord.Interaction):
        tf = interaction.data["values"][0]
        view = OutcomeActionView(self.pair, tf)
        await interaction.response.edit_message(
            content=f"📋 **{self.pair}** `{tf.upper()}` | Pilih arah trade:", view=view
        )
...
class OutcomeActionView(View):
    ...
    async def buy(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.response.edit_message(
            content=f"📋 **{self.pair}** `{self.tf.upper()}` `BUY` | Pilih hasil:", view=view
        )
    ...
    async def sell(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.response.edit_message(
            content=f"📋 **{self.pair}** `{self.tf.upper()}` `SELL` | Pilih hasil:", view=view
        )


class OutcomeResultView(View):
    """Step 4: TP1 / TP2 / SL1 / SL2 / MANUAL."""
    def __init__(self, pair: str, tf: str, action: str):
        ...
        self.action = action
        options = [
            discord.SelectOption(label="✅ TP1 — Quick profit",   value="TP1"),
            discord.SelectOption(label="✅ TP2 — Full target",    value="TP2"),
            discord.SelectOption(label="❌ SL1 — Tight stop hit", value="SL1"),
            discord.SelectOption(label="❌ SL2 — Anti-sweep hit", value="SL2"),
            discord.SelectOption(label="🔒 Manual Close",         value="MANUAL_CLOSE"),
        ]
    ...
    async def on_result(self, interaction: discord.Interaction):
        ...
        exit_map   = {
            "TP1": tp1_ref, "TP2": tp2_ref,
            "SL1": sl1_ref, "SL2": sl2_ref,
            "MANUAL_CLOSE": entry_ref,
        }
        exit_price = exit_map.get(outcome, entry_ref)

        pips_result = (exit_price - entry_ref) if self.action == "BUY" else (entry_ref - exit_price)
        rr_achieved = 0.0
        if sl2_ref and abs(entry_ref - sl2_ref) > 0:
            rr_achieved = abs(exit_price - entry_ref) / abs(entry_ref - sl2_ref)
        ...
        emoji = "✅" if win else "❌"
        embed = discord.Embed(
            title = f"{emoji} Outcome — {self.pair} {self.tf.upper()}",
        ...
        embed.add_field(
            ...
            value = (
                ...
                f"{'🏆 Win' if win else '📉 Loss'} — ML updated"
            ),
            inline=True,
        )
...
class RegimePairView(View):
    ...
    async def on_pair(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            content=f"🔍 Regime — **{pair}** | Pilih timeframe:", view=view
        )
...
class RegimeTFView(View):
    ...
    async def on_tf(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = discord.Embed(
                title = f"{regime['emoji']} Regime — {pair} {tf.upper()}",
...
# ==================================================
# MAIN MENU VIEW  (lengkap dengan semua fitur)
# ==================================================
...
class MainMenuView(View):
    ...
    @discord.ui.button(label="📈 Signal / Pair", style=discord.ButtonStyle.primary, row=0)
    ...
    @discord.ui.button(label="📊 Chart / Pair", style=discord.ButtonStyle.primary, row=0)
    ...
    @discord.ui.button(label="🔁 Scan Semua", style=discord.ButtonStyle.secondary, row=0)
    ...
    async def outcome(self, interaction: discord.Interaction, button: Button):
        view = OutcomePairView()
        await interaction.response.send_message(
            "📋 Catat hasil trade — pilih pair:", view=view, ephemeral=True
        )

    @discord.ui.button(label="🔍 Regime", style=discord.ButtonStyle.secondary, row=1)
    ...
    @discord.ui.button(label="📡 Div Status", style=discord.ButtonStyle.secondary, row=1)
    async def div_status(self, interaction: discord.Interaction, button: Button):
        ...
        if not lines:
            lines = ["Belum ada RSI Divergence dikirim sejak bot start."]
        embed = discord.Embed(
    ...
    @discord.ui.button(label="🗄 DB Stats", style=discord.ButtonStyle.secondary, row=1)
    ...
    # ── Row 2: System ─────────────────────────────
    @discord.ui.button(label="⚙️ Auto Signal", style=discord.ButtonStyle.danger, row=2)
    ...
    @discord.ui.button(label="🌐 Data Source", style=discord.ButtonStyle.secondary, row=2)
    async def datasource(self, interaction: discord.Interaction, button: Button):
        _check_tv_available()
        source = "🟢 **TradingView WS**" if _tv_ok else "🟡 **Yahoo Finance** (fallback)"
        cache  = f"Cache: `{len(_data_cache)}` entries | TTL: `{CACHE_TTL}s`"
        await interaction.response.send_message(
...
class TimeframeSelectMenu(View):
    ...
    async def on_select(self, interaction: discord.Interaction):
        ...
        path  = await run_blocking(
            ...
            best["timeframe"], best["sweep"], best["fvg"],
            best["tp1"], best["sl1"], best["rr1"], best["rr2"]
        )
